...
let fileSlug = page;
  ...
  function buildSectionItem(section, index) {
    ...
    codeBtn.addEventListener('click', e => {
      e.stopPropagation();
      window.open(`/admin/themes/edit-code.php?section=${section.type}`, '_blank');
    });
        ...
        case "list":
          {
            ...
            if (!Array.isArray(section.settings[setting.id])) section.settings[setting.id] = [];
            const list = section.settings[setting.id];
            function renderItems() {
              ...
              list.forEach((item, idx) => {
                ...
                const fields = setting.item?.properties || setting.item?.fields || [setting.item];
                fields.forEach(f => {
                  if (!f) return;
                  ...
                  if (f.type === 'textarea' || f.type === 'richtext') {
                    inpField = document.createElement('textarea');
                    inpField.value = item[f.id] || '';
                  } else if (f.type === 'image' || f.type === 'image_picker') {
                    inpField = document.createElement('input');
                    inpField.type = 'file';
                    inpField.accept = 'image/*';
                    let prev = null;
                    if (item[f.id]) {
                    ...
                    inpField.addEventListener('change', async () => {
                      if (!inpField.files.length) return;
                      const fd = new FormData();
                      fd.append('image', inpField.files[0]);
                      const r = await fetch('/backend/themes/upload-image.php', { method: 'POST', body: fd });
                      const d = await r.json();
                      if (d.success) {
                        item[f.id] = '/uploads/themes/images/' + d.filename;
                        if (!prev) { prev = document.createElement('img'); prev.style.maxWidth = '100%'; itemDiv.insertBefore(prev, inpField); }
                        prev.src = item[f.id];
                        layoutDirty = true;
                        updatePreview();
                      }
                    ...
                    inpField = document.createElement('input');
                    inpField.type = f.type || 'text';
                    inpField.value = item[f.id] || '';
                  }
                  inpField.addEventListener('input', () => {
                    item[f.id] = inpField.value;
                    layoutDirty = true;
                    updatePreview();
                  });
                  itemDiv.appendChild(l);
                  itemDiv.appendChild(inpField);
                });
            ...
            addBtn.addEventListener('click', () => {
              const fields = setting.item?.properties || setting.item?.fields || [];
              const obj = {};
              fields.forEach(f => { obj[f.id] = f.default || ''; });
              if (!fields.length) obj.value = '';
              list.push(obj);
              layoutDirty = true;
              renderItems();
              updatePreview();
